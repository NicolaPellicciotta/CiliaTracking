function varargout = cilia_clicky_thingy_GUI(varargin)
...
gui_State = struct('gui_Name',       mfilename, ...
    ...
    'gui_OpeningFcn', @cilia_clicky_thingy_GUI_OpeningFcn, ...
    'gui_OutputFcn',  @cilia_clicky_thingy_GUI_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
function cilia_clicky_thingy_GUI_OpeningFcn(hObject, eventdata, handles, varargin)
...
% Opens file
[handles.filename, handles.filepath] = uigetfile('*.movie');
%debug
...
handles.movie_object = moviereader(fullfile(handles.filepath,handles.filename));
handles.fc = 1; %frame counter
handles.total_frames.String = ['/',num2str(handles.movie_object.NumberOfFrames)];
...
update_updateable(hObject, handles);
...
function varargout = cilia_clicky_thingy_GUI_OutputFcn(hObject, eventdata, handles)
...
function edit_clickrate_Callback(hObject, eventdata, handles)
...
function edit_clickrate_CreateFcn(hObject, eventdata, handles)
% hObject    handle to edit_clickrate (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
...
function delete_frame_points_Callback(hObject, eventdata, handles)
...
function click_points_Callback(hObject, eventdata, handles)
...
if isempty(handles.clicked_frames(handles.fc).IM)
    [im,ts] = handles.movie_object.read(handles.fc);
...
function update_updateable(hObject, handles)
update_frame_and_points(hObject, handles)
...
function update_frame_and_points(hObject, handles)

if handles.alternative_view == false
    imshow(imadjust(handles.movie_object.read(handles.fc)),[]);
else %if toggle for alternative view is switched on
    cf = double(handles.movie_object.read(handles.fc)); % current frame
    
    ds = max(0, handles.fc - floor(handles.avint/2));
    de = min(handles.movie_object.NumberOfFrames, handles.fc + floor(handles.avint/2));
    dfs = double(handles.movie_object.read([ds,de])); %read short stack
...
function alternative_view_Callback(hObject, eventdata, handles)
