...
for i = flip(idx_clicked_frames)
    ...
    %     if flag_debugging,
    figure(hf1);
    plot(clclk.points(i).dw_cilium_xx, clclk.points(i).dw_cilium_yy,'b');
...
for lc = numel(Line):-1:1 % sneaky allocation
    ...
    %     if flag_debugging,
    figure(hf1);
    hpi(lc) = plot(clclk.Intersections(lc).line_x, clclk.Intersections(lc).line_y, 'm'); %handle plot intersection
